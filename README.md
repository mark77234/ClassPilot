...
- 팀 점수 추가/삭제, 물리 기반 마블 룰렛, 우승 상품 설정, 최종 우승팀 발표
- 교실 보드에서 학생 드래그 이동 및 다른 학생 위 드롭 시 자리 교환
- 학생별 특징 태그, 메모, 상점/벌점 기록
- 팀 점수 추가/삭제
- 마블 룰렛: 팀/학생별 공을 생성하고 완주 순서대로 점수표 표시
...
- `Student`: id, name, position, traits, memo, merit, demerit
...
- `StudentPointEvent`: id, studentId, studentName, kind, points, reason, createdAt
- `FinaleState`: finished, winnerTeamId, finishedAt
- `ClassSession`: className, appStep, mainSection, students, teams, polls, timer, reward, scoreEvents, studentPointEvents, finale
